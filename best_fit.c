...
struct node* allocate(struct node* head){
    ...
    if(head == NULL){
        ...
        if(n->size_in_kb <= head->start_add){
            if(head->start_add < min_space){
                //if the space is minm
                min_space = head->start_add;
                s = NULL;
                e = head;
            }
        }
        ...
        while(q!=NULL){
            if(n->size_in_kb <= (q->start_add - (p->start_add + p->size_in_kb))){
                //Check if gap is minimum or not
                if(q->start_add - (p->start_add + p->size_in_kb) < min_space){
                    min_space = q->start_add - (p->start_add + p->size_in_kb);
                    s = p;
                    e = q;
                }
            }
        ...
        if(q == NULL){
            //Checking for gap at end,if it's sufficicent and minm aur not.
            if(n->size_in_kb <= (total_memory_size - (p->start_add + p->size_in_kb)) ){
                //sufficient space is there after last node.
                if(total_memory_size - (p->start_add + p->size_in_kb) < min_space){
                    //it's the minimum space
                    min_space = total_memory_size - (p->start_add + p->size_in_kb);
                    s = p;
                    e = NULL;
                }
            }
    ...
    if(s == NULL && e == NULL){
        ...
        //best fit before the first node
        n->start_add = 0;
        n->next = e;
        head = n;
        traverse_linked_list(head);
